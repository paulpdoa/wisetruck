...
const CollectorChangePassword = () => {

    const { id } = useParams();
    const { records } = fetchApiHook(`${baseUrl()}/collectors/${id}`);
    ...
    const updatePassword = async (e) => {
        ...
        if(password.length >= 8) {
            if(password === confirmPassword) {
                try {
                    const data = await axios.patch(`${baseUrl()}/collector/update/password/${id}`,{password});
...
export default CollectorChangePassword;